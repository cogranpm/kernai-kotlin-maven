...
object PublicationMapper : IMapper<Publication> {
    ...
    override fun getAll(keys: Map<String, Long>): List<Publication> {
        return MapperHelper.getAll(
            keys,
            table,
            table.subjectId eq keys["subjectId"] as Long, table.title to SortOrder.ASC
        ) {
            Publication(
                it[table.id].value,
                it[table.subjectId],
                it[table.title],
                it[table.type] ?: LookupUtils.getLookupByKey(LookupUtils.publicationTypeLookupKey, false)[0].code,
                it[table.comments] ?: "",
